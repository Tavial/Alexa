...
def handle (handler_input):
    ...
    if tamanyo > 1: # el usuario está preguntando por un municipio en el que hay varios que se llaman parecido. Ej: pregunta por torrejón y hay tres torrejones, torrejón de Ardoz, Torrejón de Velasco y Torrejón de la Calzada
        ...
        handler_input.attributes_manager.session_attributes["set_casos_totales_municipio"] = True
    elif tamanyo == 1: 
        ...
        if datos_covid.status_code == 200:
            ...
            if len(datos) == 0:
                ...
                for i in range (199):
                    if 'municipio_distrito' in datos[i] and 'casos_confirmados_totales' in datos[i] and datos[i]["municipio_distrito"] == municipio:
                        speak_output = "En "+municipio+" se han confirmado un total de "+str(datos[i]['casos_confirmados_totales'])+" casos desde el inicio de la pandemia."
        else: 
